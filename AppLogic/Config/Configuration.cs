...
namespace AppLogic.Config
{
    ...
    internal static class Configuration
    {
        ...
        static Configuration()
        {
            ...
            _localConfig = new Lazy<System.Configuration.Configuration>(() => ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None), false);

            _roamingConfig = new Lazy<System.Configuration.Configuration>(() => ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.PerUserRoaming), false);

            const string optionsSection = "options";
            _localOptions = new Lazy<OptionCollection>(() => _localConfig.Value.GetCustomSection<OptionCollection>(optionsSection), false);

            _roamingOptions = new Lazy<OptionCollection>(() => _roamingConfig.Value.GetCustomSection<OptionCollection>(optionsSection), false);

            const string hotkeysSection = "hotkeys";
            _localHotkeys = new Lazy<HotkeyCollection>(() => _localConfig.Value.GetCustomSection<HotkeyCollection>(hotkeysSection), false);

            _roamingHotkeys = new Lazy<HotkeyCollection>(() => _roamingConfig.Value.GetCustomSection<HotkeyCollection>(hotkeysSection), false);
        }
